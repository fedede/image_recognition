...
features = zeros(length(Xtrain),6);
...
for i = 1:length(Xtrain)
    ...
    features(i,5) =  area/box_area;
    features(i,6) = perimeter/box_perimeter;
...
features_n = zeros(length(Xtrain),6);
...
for i=1:size(features,1)
    ...
    features_n(i,5) = (features(i,5) - feat_mean(5)) / feat_std(5);
    features_n(i,6) = (features(i,6) - feat_mean(6)) / feat_std(6);
end
...
% Creating empy array of features
features_test = zeros(length(Xtest),6);
for i = 1:length(Xtest)
    ...
    [area, perimeter, box_area, box_perimeter] = get_object_props(mask);
    % Extract shape feature for detected object
    features_test(i,5) =  area/box_area;
    features_test(i,6) =  perimeter/box_perimeter;
...
features_test_n = zeros(length(Xtest),6);
...
for i=1:size(features_test,1)
    ...
    features_test_n(i,5) = (features_test(i,5) - feat_mean(5)) / feat_std(5);
    features_test_n(i,6) = (features_test(i,6) - feat_mean(6)) / feat_std(6);
end
